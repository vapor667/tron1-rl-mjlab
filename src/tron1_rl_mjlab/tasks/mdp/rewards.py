...
def _get_command_task_mask(
    ...
    is_velocity_env = getattr(command_term, "is_velocity_env", None)
    if is_velocity_env is None:
        return torch.ones(env.num_envs, device=env.device)

    if task_mode == "velocity":
        return is_velocity_env.float()
...
def _get_velocity_commands(
    ...
    command_term = env.command_manager.get_term(command_name)
    if hasattr(command_term, "pose_command_vel_c"):
        return command_term.pose_command_vel_c
...
def safety_reward_exp(
    ...
    reward = mani_safety_scale * 0.5 + loco_safety_scale * 0.5
    return reward * _get_command_task_mask(env, command_name, task_mode)
...
def track_base_position_exp(
    ...
    reward = (normal + micro_enhancement) * 0.5 * env._loco_safety_scale
    return reward * _get_command_task_mask(env, command_name, task_mode)
...
def track_base_orientation_exp(
    ...
    reward = (normal + micro_enhancement) * position_scale * 0.5 * env._loco_safety_scale
    return reward * _get_command_task_mask(env, command_name, task_mode)
...
def track_base_pb(
    ...
    reward = (2 * pos_improve * position_scale + orient_improve * orient_scale) * env._loco_safety_scale
    return reward * _get_command_task_mask(env, command_name, task_mode)
...
def track_base_reference_exp(
    ...
    reward = torch.exp(-track_error / std ** 2) * 0.5 * env._loco_safety_scale
    return reward * _get_command_task_mask(env, command_name, task_mode)
...
def stand_still_velocity(
    ...
    asset: Entity = env.scene[asset_cfg.name]
    commands = _get_velocity_commands(env, command_name)
...
def stay_alive_task(
    ...
    return stay_alive(env) * _get_command_task_mask(env, command_name, task_mode)


def lin_vel_z_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return lin_vel_z_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)


def ang_vel_xy_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return ang_vel_xy_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)


def joint_vel_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return joint_vel_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)


def joint_torques_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return joint_torques_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)


def joint_acc_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return joint_acc_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)


def action_rate_l2_task(
        env: ManagerBasedRlEnv,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return action_rate_l2(env) * _get_command_task_mask(env, command_name, task_mode)
...
def joint_pos_limits_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return joint_pos_limits(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)
...
def undesired_contacts_task(
    ...
    return undesired_contacts(env, sensor_cfg=sensor_cfg, threshold=threshold) * _get_command_task_mask(
        env, command_name, task_mode
    )
...
def flat_orientation_l2_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return flat_orientation_l2(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)
...
def base_com_height_task(
    ...
    return base_com_height(env, target_height=target_height, asset_cfg=asset_cfg) * _get_command_task_mask(
        env, command_name, task_mode
    )
...
def feet_distance_task(
    ...
    return feet_distance(
        env,
        asset_cfg=asset_cfg,
        feet_links_name=feet_links_name,
        min_feet_distance=min_feet_distance,
        max_feet_distance=max_feet_distance,
    ) * _get_command_task_mask(env, command_name, task_mode)
...
def leg_symmetry_task(
    ...
    return leg_symmetry(env, std=std, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)
...
def same_feet_x_position_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return same_feet_x_position(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)
...
def joint_powers_l1_task(
        env: ManagerBasedRlEnv,
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return joint_powers_l1(env, asset_cfg=asset_cfg) * _get_command_task_mask(env, command_name, task_mode)
...
def weighted_joint_torques_l2_task(
        env: ManagerBasedRlEnv,
        torque_weight: dict[str, float],
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
    ...
    return weighted_joint_torques_l2(env, torque_weight=torque_weight, asset_cfg=asset_cfg) * _get_command_task_mask(
        env, command_name, task_mode
    )
...
def weighted_joint_power_l1_task(
        env: ManagerBasedRlEnv,
        power_weight: dict[str, float],
        asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
        command_name: str = "base_pose",
    ...
    return weighted_joint_power_l1(env, power_weight=power_weight, asset_cfg=asset_cfg) * _get_command_task_mask(
        env, command_name, task_mode
    )
...
def action_smoothness_penalty_task(
        env: ManagerBasedRlEnv,
        command_name: str = "base_pose",
        task_mode: str | None = None,
) -> torch.Tensor:
    return action_smoothness_penalty(env) * _get_command_task_mask(env, command_name, task_mode)
